...
class Utility(commands.Cog): 
    ...
    async def CreateTicket(self,ctx,members:commands.Greedy[discord.Member], *, description='Trade Channel'):
        if ctx.guild.id in config.APPROVED_SERVERS:
            ...
            embed.add_field(name="Process",value="The selling party would send the disc to the buying party. Meanwhile the buying party can transfer a Game Trade Moderator the money. Once the buyer gets the product and checks the condition, the Game Trade Moderator would send the money to the seller.",inline=False)
            embed.add_field(name="Seller Instructions",value="Confirm the product and it's price before proceeding further.",inline=False)
