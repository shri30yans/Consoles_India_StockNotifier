...
class NotificationHandler:
    """Formats rule matches and dispatches them to the configured channels."""

    def __init__(
        self,
        router: ChannelRouter,
        affiliate_rewriter: AffiliateRewriter | None = None,
    ) -> None:
        self._router = router
    ...
    async def handle_rule_match(self, match: RuleMatch) -> None:
        ...
        for channel_id in notification.target_channels:
            try:
                ...
                logger.exception("Failed to send to channel %s: %s", channel_id, e)
